...
def download(row):
    u = row["url"]
    if pd.isna(u): return
    name = (str(row.get("md5hash")).strip()+".jpg") if not pd.isna(row.get("md5hash")) and str(row.get("md5hash")).strip() else Path(str(u)).name
    p = out/name
    for _ in range(3):
        try:
            with requests.Session() as s:
                r = s.get(u, timeout=30, stream=True, headers={"User-Agent":"Mozilla/5.0"})
                r.raise_for_status()
                with open(p, "wb") as f:
